...
The primary difference is that rather than embedding groups of aligned feature vectors to a shared Hamming space, we are embedding entire unaligned sequences as fixed-length vectors.
...
\end{equation}
However, the learned embedding functions $f$ and $g$ will produce a single vector, and their input will be entire unaligned sequences rather than short groups of aligned feature vectors.
...
Specifically, we will utilize two different embedding networks for $f$ and $g$, each of which will consist of a feed-forward attention mechanism followed by fully-connected feedforward layers.
For the attention mechanism's learned adaptive weighting function $a$, we use the standard architecture of a small neural network with one hidden layer:
\begin{equation}
a(x) = v^\top\tanh(Wx + b)
...
\begin{figure}
  \includegraphics[width=\textwidth]{figures/5-model_schematic.pdf}
...
We optimized the the learning rate and RMSProp decay parameter, and the loss parameters $\alpha$ and $m$.
...
\item The output $L^2$ penalty from \cref{ch:dhs} as well as the the hash entropy-encouraging loss term proposed in \cite[equation (3)]{yang2015supervised}, but neither were beneficial.
\item Using dropout in the fully-connetcted layers, which hurt performance.
...
\begin{figure}
  \includegraphics[width=\textwidth]{figures/5-embeddings.pdf}
  \caption[Example embeddings for two pairs of sequences]{Example embeddings for two pairs of matching sequences produced by our feedforward attention model after training.
...
From this fiture, we can see that our proposed approach produces embeddings which better differentiate between matching and non-matching pairs.
...
This means that we can discard over 90\% of the MSD with high confidence with an operation which is xxx times faster than the downsampled hash sequence-based methods of \cref{ch:dhs} and is xxx times faster than a brute-force DTW search using the approach from \cref{ch:dtw}.
