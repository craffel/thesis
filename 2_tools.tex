...
In either case, adjusting the parameters $\theta$ of $f$ typically involves using a task-specific ``loss function'' $\mathcal{L}$ which computes a non-negative scalar value which measures the extent to which the model's output $\hat{y}$ agrees with the target $y$.
...
\end{tabular}
\end{center}
\caption[Common loss functions used in neural networks]{Common loss functions $\mathcal{L}(a_L, y)$ used in neural networks and their derivatives with respect to their input $a_L$.}
...
Dropout intends to prevent the units of a given layer from being too heavily correlated with one another randomly artificially removing connections.
