...
Three of the most important psychoacoustic charasterics of audio signals are the pitch, loudness, and timbre.
...
\begin{equation}
        S[n, k] = \sum_{m = 1}^M w[m]x[m + nR - M/2] e^{-2\pi \jmath (k - 1)(m - 1)/M}, k \in \{1, \ldots, M\}
\end{equation}
...
A common choice is a constant-Q transform (CQT), which has frequency bins which are logarithmically spaced and all have equal center frequencies-to-bandwidth ratios \cite{}.
Fortunately, like the DFT, fast algorithms exist for computing the CQT \cite{} \cite{}.

In this thesis, we will utilize the transform proposed by \cite{}, which is defined as follows:
\begin{equation}
        S[n, k] = \frac{1}{M_k} \sum_{m}^{M_k} w_k[m]x[m - M_k/2 + nR] e^{2\pi \jmath m f_k/f_s}
\end{equation}
