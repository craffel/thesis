...
In order to extend this method to sequential data, a natural approach would be to compute the sum of the distances of co-occurring elements in two sequences.
...
While the application of dynamic programming makes many warping-based measures tractible, they can be made inpractical in practice by a variety of factors.
...
To motivate the central problem of this thesis, in \cref{ch:midi} we discuss the various sources of information available in MIDI files and provide a broad overview of the availability of this information in the 178,561 MIDI files we obtained through a large-scale web scrape.
...
To wrap up, we describe some of the characteristics of our new dataset and outline some interesting applications for it.
...
For visualization, we will use the following color pallete:
\begin{center}
\includegraphics[width=.5\textwidth]{figures/1-colors.pdf}
\end{center}
In captions, we will refer to these colors (from left to right) as ``blue'', ``green'', ``gray'', ``orange'', and ``red''.
