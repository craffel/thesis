...
Using the combined method, the correct match in the MSD had the highest confidence score xxx\% of the time, and for xxx\% of the test set the correct match ranked in the top xxx.
Beyond this point, the performance is constant - this is because for xxx\% of the test set, the pruning steps discarded the correct match.
This is within our expected upper limit for the false reject rate of xxx\%.
That our high-precision approach only produced the correct match xxx\% of the time is also expected, due to the fact that the MSD contains many duplicates and alternate versions of the songs \cite{ellis2012large}.
...
Compared to the approximately 1.5 days it would take to match a single MIDI file to the MSD using the approach from \cref{sec:goldstandard} on its own, we consider the xxx false reject rate a small price to pay for this increase in efficiency.
